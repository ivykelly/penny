...
export default function AnswerResponse({ isCorrect, show, onContinue, correctAnswer, currentCoins }: AnswerResponseProps) {
    ...
    const getMessage = () => {
        if (isCorrect) {
            ...
            return "Correct! +20 coins!"; // Return the correct message
        }

        if (currentCoins === 0) {
            // If the user's coins are 0
            return "Incorrect."; // Return the incorrect message
        }

        if (currentCoins <= 10) {
            // If the user's coins are less than or equal to 10
    ...
    return (
        // Return the AnswerResponse component
